#include "ColorCycle.h"

#include <Arduino.h>
#include <cmath>

#include "RGB.h"

namespace Color {

ColorCycle::ColorCycle(uint16_t length, uint16_t rOffset, uint16_t gOffset, uint16_t bOffset) :
		_length(length), _colors(new RGB[length]) {
	for (int i = 0; i < length; i++) {
		_colors[i].red = 127.5 * (1.0 + sin(2.0 * PI * (i + rOffset) / length));
		_colors[i].green = 127.5 * (1.0 + sin(2.0 * PI * (i + gOffset) / length));
		_colors[i].blue = 127.5 * (1.0 + sin(2.0 * PI * (i + bOffset) / length));
	}
}

RGB ColorCycle::get(uint16_t pos) const {
	return _colors[pos % _length];
}

} /* namespace Color */
